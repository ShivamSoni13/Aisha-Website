...
function StickyCTA() {
  return (
    <motion.div
      initial={{ y: 80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 1.2, duration: 0.4, ease: "easeOut" }}
      style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 90, background: "rgba(255,255,255,0.97)", backdropFilter: "blur(16px)", borderTop: "1px solid var(--slate-200)", padding: "10px 20px 14px" }}
    >
      <div style={{ maxWidth: 700, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <div style={{ width: 7, height: 7, background: "#22c55e", borderRadius: "50%", animation: "blink 2s infinite" }} />
          <p style={{ fontFamily: "var(--font-body)", fontSize: 13, color: "var(--slate-600)", margin: 0 }}>
            <strong style={{ color: "var(--slate-900)" }}>Limited slots</strong> — Call or book online · Limited Availability
          </p>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <motion.a href="https://calendly.com/booksmiledentalcare/clear-aligners-open-day-sat-4th-july-with-dr-aisha"
            whileHover={{ y: -1 }} whileTap={{ scale: 0.96 }}
